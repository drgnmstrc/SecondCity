...
/datum/world_topic/plx_announce/Run(list/input)
	...
	send_ooc_announcement(message, "From [from]")
...
/*
		if("mhelp")
			// aticket_closed call inside of the below proc
			ticket.convert_to_mentorhelp(action_by_ckey, force = TRUE)
*/
...
/datum/world_topic/plx_sendaticketpm/Run(list/input)
	...
	if(!QDELETED(recipient))
		...
		recipient.externalreplyamount = 2
