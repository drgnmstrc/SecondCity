# The modularization handbook - Nova Sector style, v0.3
...
```diff
- var/something = 1
+ var/something = 2 // CRIMSON EDIT CHANGE - ORIGINAL: var/something = 1
```
...
```byond
var/something = 2 // CRIMSON EDIT CHANGE - ORIGINAL: var/something = 4
```
...
And then you'll want to establish your core folder that you'll be working out of which is normally your module ID. E.g. `modular_vcg/modules/shuttle_toggle`
...
We will no longer have \_vcg map versions.
...
- **_Example:_** You're adding a new lavaland mob.
  ...
  First of all you create your modular folder. E.g. `modular_vcg/modules/lavalandmob`
  ...
  ```byond
    /mob/lavaland/newmob
      icon = 'modular_vcg/modules/lavalandmob/icons/mob.dmi'
      icon_state = "dead_1"
      sound = 'modular_vcg/modules/lavalandmob/sounds/boom.ogg'
  ```
...
Example: `code/modules/mob/living/living.dm` -> `modular_vcg/master_files/code/modules/mob/living/living.dm`
...
In short, most of the modular code will be placed in the subfolders of your main module folder **`modular_vcg/modules/yourmodule/code/`**, with similar rules as with the assets. Do not mirror core code folder structures inside your modular folder.

For example, `modular_vcg/modules/xenoarcheaology/code` containing all the code, tools, items and machinery related to it.
...
In general they are a good idea and encouraged whenever it is possible to do so. However this is not a hard rule, and sometimes CRIMSON edits are preferable. Just try to use your common sense about it.
...
Performance should also be considered when you are overriding a hot proc (like Life() for example), as each additional call adds overhead. CRIMSON edits are much more performant in those cases. For most procs this won't be something you have to think about, though.
...
- **Addition:**
  ...
  ```byond
  // CRIMSON EDIT ADDITION START - SHUTTLE_TOGGLE - (Optional Reason/comment)
...
- **Removal:**
  ...
  /* // CRIMSON EDIT REMOVAL START - SHUTTLE_TOGGLE - (Optional Reason/comment)
  ...
  *Please avoid this however, only to be done if there is no other option.
  ...
  ```byond
  /* // CRIMSON EDIT REMOVAL START - SHUTTLE_TOGGLE - (Moved to modular_vcg/shuttle_toggle/randomverbs.dm)
...
- **Change:**
  ...
  ```byond
  if(SHUTTLE_STRANDED, SHUTTLE_ESCAPE, SHUTTLE_DISABLED) // CRIMSON EDIT CHANGE - ORIGINAL: if(SHUTTLE_STRANDED, SHUTTLE_ESCAPE)
  ```
  ...
  ```byond
  /* // CRIMSON EDIT REMOVAL START - Adds conditional
  	return 1
  */ // CRIMSON EDIT REMOVAL
  // CRIMSON EDIT ADDITION START - Adds conditional
  	if(!isnull(src))
  		return 1
  // CRIMSON EDIT ADDITION END
...
That folder is **`code/__DEFINES/~crimson_defines`**, in which you can add them to the existing files, or create those files as necessary.
...
- Good: /modular_vcg/modules/example_module/code/disease_mob.dm
- Bad: /modular_vcg/modules/example_module/code/modules/antagonists/disease/disease_mob.dm

**Icons**: Any .DMI files must go in here.

- Good: /modular_vcg/modules/example_module/icons/mining_righthand.dmi
- Bad: /modular_vcg/modules/example_module/icons/mob/inhands/equipment/mining_righthand.dmi
...
ALL of the tgui files are located in `/tgui/packages/tgui/interfaces` and its subdirectories; there is no specific folder for CRIMSON Sector UIs.
...
You can do both `// CRIMSON EDIT` and `/* CRIMSON EDIT */`, though in some cases you may have to use one over the other.
...
<Button
	...
	icon="rat" // CRIMSON EDIT ADDITION
</Button>
```
...
<Button
	onClick={() => act('spin', { high_quality: true })}
	// CRIMSON EDIT ADDITION START - another example, multiline changes
	icon="rat"
...
This way they are easily identifiable as modular TGUI .tsx/.jsx files. You do not have to do anything further, and there will never be any need for a CRIMSON edit comment in a modular TGUI file.

#### Option 2: Folder Structuring

Alternatively, you can create a subfolder within the directory where you are making changes and name it `crimson`, if it seems fit for this circumstance.

- Place all new files related to your changes inside this `crimson` folder.
