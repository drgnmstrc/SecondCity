...
/datum/tgs_chat_command/validated/Run(datum/tgs_chat_user/sender, params)
	...
	var/discord_id = SSplexora.get_discord_id_from_mention(sender.mention) || sender.id // CRIMSON EDIT CHANGE - SSdiscord -> SSplexora
	if (!discord_id)
		return new /datum/tgs_message_content("Error: Unknown error trying to get your discord id.")

	var/datum/admins/linked_admin
	var/admin_ckey = ckey(SSplexora.lookup_ckey(discord_id)) // CRIMSON EDIT CHANGE - SSdiscord -> SSplexora
